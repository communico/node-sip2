...
class Connection {
	...
	connect(callback) {
				...
				this.socket.connect(this.proxyPort, this.proxyHost, () => {
					...
					this.socket.once('data', (data) => {
						if (data.toString('hex') === "0500") {
							...
							this.socket.end();
							onConnected("Socks5 handshake failed. Closing the socket");
						}
					});
				});
			} else {
				this.socket.connect(this.port, this.host, onConnected);
				this.socket.setEncoding('utf8');
		...
		doConnect((err) => {
			...
			if(this.sslOptions.enabled) {
				this.createTlsConnection(callback, this.socket);
			} else {
				callback();
			}
		});
	...
	createTlsConnection(callback, preWrappedSocket = null) {
		const options = {
		  host: this.host,
		  socket: preWrappedSocket,
		  ...this.sslOptions
		};
